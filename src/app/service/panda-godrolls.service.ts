...
export interface clarityStyles {
  'background': string;
  'blue': string;
  'bold': string;
  'breakSpaces': string;
...
const joinClassNames = (classNames?: (keyof typeof clarityStyles)[]) =>
  classNames?.map((className) => clarityStyles[className]).join(' ');
...
export default function ClarityDescriptions({
  ...
  const selectedLanguage = "en"; //useSelector(languageSelector);
  ...
  return (
    "<div className={clsx(clarityStyles.communityDescription, className)}> \
      <h3>CommunityData</h3> \
